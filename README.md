...
- **Sidebar navigation** — icon + label nav for Dashboard, Notes, Study, and Roblox lookup. Click the thin strip on the sidebar's right edge to collapse it to icons-only; the state is remembered per browser (and defaults to collapsed the first time it's opened on a phone-sized screen).
...
If chat or the Roblox lookup show an error, the message includes the actual HTTP status/detail instead of a generic one — check that text first, then the **Vercel Dashboard → your project → Logs** for the full server-side error. If you ever see a raw "NOT_FOUND" page (not this app's own error text) for any `/api/*` route, that means Vercel isn't finding a deployed function there at all — check the deployment's **Functions** tab lists `auth`, `data`, `roblox`, and `chat`, and that the latest commit actually deployed (Vercel dashboard → Deployments → confirm the top one matches your latest push and shows "Ready", not a failed build).

## Stack

Static HTML/CSS/vanilla JS frontend, plus four serverless functions (all plain CommonJS `.js`, no build step, no middleware):
